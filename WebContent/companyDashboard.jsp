...
<%
	...
	mResult = (ResultSet) DatabaseConnector
			.getResultSet("select job_title from post_job_td where user_id = " + user_id + ";", 1);
	ArrayList<String> querySubject = new ArrayList<String>();
	while (mResult.next()) {
		querySubject.add(mResult.getString("job_title"));
	}

	//getting the total companies in the system
	String appliedJobCount = "";
	mResult = (ResultSet) DatabaseConnector.getResultSet(
			"SELECT count(*)FROM post_job_td INNER JOIN applied_job ON post_job_td.job_id = applied_job.job_id;",
			1);
	if (mResult.next()) {
		appliedJobCount = mResult.getString("COUNT(*)");
	}
	...
	String queriesCount = "";
	mResult = (ResultSet) DatabaseConnector
			.getResultSet("SELECT COUNT(*) FROM queries where query_to = " + user_id + ";", 1);
	if (mResult.next()) {
	...
	//getting the total companies in the system
	String jobPostedCount = "";
	mResult = (ResultSet) DatabaseConnector
			.getResultSet("SELECT COUNT(*) FROM post_job_td where user_id = " + user_id + ";", 1);
	if (mResult.next()) {
		jobPostedCount = mResult.getString("COUNT(*)");
	}
...
<body>
	...
	<a href="#"
		...
		Dashboard</a> <a href="postJob.jsp" class="w3-padding"><i
		class="fa fa-eye fa-fw"></i>  Post Job</a> <a href="listOfPostedJobs.jsp" class="w3-padding"><i
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<form action="AppliedStudents" method="post">
