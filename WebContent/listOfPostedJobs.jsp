...
<%@ page import="pojo.JobPostedByCompanyPojo"%>
...
<%
	...
	mResult = (ResultSet) DatabaseConnector
			.getResultSet("select * from post_job_td where user_id = " + user_id + ";", 1);
	ArrayList<JobPostedByCompanyPojo> querySubject = new ArrayList<JobPostedByCompanyPojo>();
	while (mResult.next()) {
		JobPostedByCompanyPojo mPojo = new JobPostedByCompanyPojo(mResult.getString("job_id"),
				mResult.getString("job_title"));
		querySubject.add(mPojo);
	}
%>
		...
		<div class="col-md-11">
			<table class="table">
				<tbody>
					...
					%>
					<tr>
						<td class="txt"><%=querySubject.get(i).getJob_title()%></td>
						<td>
							<form action="AppliedStudents" method="post">
								<input type="hidden" name="job_id"
									value="<%=querySubject.get(i).getJob_id()%>">
								<button type="submit" style="float: right;"
									class="btn btn-primary">View Applicants</button>
							</form>
						</td>
					</tr>
					<%
